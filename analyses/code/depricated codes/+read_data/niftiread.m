...
narginchk(1,2);
% varargin = stringToChar(varargin);

if isstruct(varargin{1}) % the info syntax.
    infoIn = varargin{1};
    if isfield(infoIn, 'Filename')
        NF = read_data.nifti.niftiFile(infoIn.Filename);
    else
        error(message('images:nifti:invalidStructSpecified'))
    end
else
    NF = read_data.nifti.niftiFile(varargin{:});
end

NV = read_data.nifti.niftiImage(NF.FileWithHeader);
